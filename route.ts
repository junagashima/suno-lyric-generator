...
`
    ...
    const lyricsCompletion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "あなたは日本の音楽業界で活躍する経験豊富な作詞家です。Suno AIに最適化された、日本のヒット曲の要素を持つ歌詞を作成することができます。J-POPの作詞戦略、リスナーの心理、現代のトレンドを深く理解しています。"
        },
        {
          role: "user",
    ...
    // タイトル候補を抽出
    const titlesMatch = lyricsResponse.match(/\*\*タイトル候補:\*\*\n((?:\d+\.\s*\[.+?\]\n?)+)/s)
    let titles: string[] = []
    if (titlesMatch) {
