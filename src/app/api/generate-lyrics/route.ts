...
`
    ...
    const lyricsCompletion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "あなたは日本の音楽業界で活躍する経験豊富な作詞家です。Suno AIに最適化された、日本のヒット曲の要素を持つ歌詞を作成することができます。J-POPの作詞戦略、リスナーの心理、現代のトレンドを深く理解しています。楽曲の雰囲気とテーマに基づいて、最適な構成を選択し、創造的で多様なパターンを使用することができます。楽曲構成は単調にならないよう、異なる構成を使用してください。**重要：指定された内容のみを歌詞に反映し、過去のリクエストや他の楽曲の要素は一切含めません。** 必ず「**タイトル候補:**」セクションから始めて、魅力的なタイトルを3つ提案し、その後に歌詞を続けてください。"
        },
        {
          role: "user",
